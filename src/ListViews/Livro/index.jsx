    ...
    handleList(dataset){
        return <SafeAreaView>
            <ScrollView refreshControl={<RefreshControl onRefresh={ _ => this.buscaDados()} />} >
                {dataset.map( item => {
                    return <>
                        <TouchableOpacity key={item} onPress={e => this.props.navigation.navigate("LivroForm", {data: item})}>
                            <View style={styles.caixa}>
                                ...
                                <View>
                                    ...
                                    <Text style={item.status == 1 ? {fontWeight: 'bold', color: 'green'} : {fontWeight: 'bold', color: 'red'}}>
                                        Status: {item.status == 1 ? 'Disponível' : 'Não disponível'} 
                                    </Text>    
                                </View>
                    ...
                    <View style={styles.container}>
                        <View style={styles.blocoPrincipal}>
                            ...
                            <View style={styles.caixaPessoas}>
                                {this.state.content}
                                <Button onPress={ _ => this.props.navigation.navigate("LivroForm")}/>
                            </View> 
                        </View>
                    </View>    
